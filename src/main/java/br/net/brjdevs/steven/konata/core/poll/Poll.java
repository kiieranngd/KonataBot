...
public class Poll implements DBObject {
    ...
    public Poll(String id, Map<String, List<String>> votes, String name, String creator, String channel, long createdAt) {
        this.votes = votes;
        this.name = name;
    ...
    public Poll(User user, TextChannel textChannel, String name, String[] options) {
        this.votes = new LinkedHashMap<>(
                Stream.of(options).collect(Collectors.toMap(s -> s, o -> new ArrayList<String>())));
        this.name = name;
    ...
    public MessageEmbed toEmbed() {
        ...
        votes.forEach((option, votes) -> embedBuilder.addField(i.incrementAndGet() + ". " + option, "`" + StringUtils.getProgressBar(votes.size(), totalVotes) + "` (" + votes.size() + " votes)", false));
        User creator = KonataBot.getInstance().getUserById(getCreator());
